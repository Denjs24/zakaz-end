...
da.init();
let selectHeaders = document.querySelectorAll('.select-catalog__head');

// for (const selectHeader of selectHeaders) {
//     selectHead = selectHeaders[]
// }
for (let selectHeader = 0; selectHeader < selectHeaders.length; selectHeader++) {
    const selectHead = selectHeaders[selectHeader];
    selectHead.addEventListener('click' , () => {
        if (selectHead.classList.contains('_active')) {
            selectHeadRemover();
            return false;
        }
        selectHeadRemover();
        let selectPerent = selectHead.closest('.catalog__item');
        // if (selectHead.classList.contains('_active')) {
        //     selectPerent.classList.remove('_active');
        //     selectHead.classList.remove('_active');
        //     return;
        // }
        let item = selectPerent.querySelector('.select-catalog__body')
        item.classList.toggle('_active')
        selectPerent.classList.toggle('_active');
        selectHead.classList.toggle('_active');
        
    })
}

function selectHeadRemover(){
    for (let index = 0; index < selectHeaders.length; index++) {
        const selectHeade = selectHeaders[index];
        let selectPerent = selectHeade.closest('.catalog__item');
        let item = selectPerent.querySelector('.select-catalog__body')
        item.classList.remove('_active')
        selectPerent.classList.remove('_active')
        selectHeade.classList.remove('_active')
    }
